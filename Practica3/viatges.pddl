(define (domain viatge)
	...
	(:action ciudad-inicio
	  ...
	  :effect (and 	(esta-a ?c)
	  			   	(increase (dias-totales) (min-dias-ciudad ?c)) 
				   	(increase (dias-ciudad ?c) (min-dias-ciudad ?c))
				   	(not (ciudad-empty ?c))
				   	(increase (actual) 1)
	...
	(:action reservar-vol-hotel
		...
		:effect (and (not (esta-a ?c1)) 
					 ...
					 (not (ciudad-empty ?c2))
					 (increase (actual) 1)
					 (increase (dias-totales) (min-dias-ciudad ?c2)) 
					 (increase (dias-ciudad ?c2) (min-dias-ciudad ?c2))
					 (increase (interes-total) (interes ?c2))
					 (increase (precio-total) (precio-hotel ?h))
					 (increase (precio-total) (precio-vuelo ?v))
				)
	)

	(:action mas-dias
	  :parameters (?c - ciudad)
	  :precondition (and (not (inicial)) (esta-a ?c) (not (>= (dias-ciudad ?c) (max-dias-ciudad ?c))))
